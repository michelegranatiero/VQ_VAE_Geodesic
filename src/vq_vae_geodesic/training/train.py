...
def fit_vae(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for image_batch, _ in batch_bar:
            ...
            train_loss_averager(loss)
            train_recon_averager(recon_loss)
            refresh_bar(
                batch_bar, f"train batch [loss: {train_loss_averager(None):.3f} | recon loss: {train_recon_averager(None):.3f}]")

        train_loss_avg_history.append(train_loss_averager(None))
        train_recon_avg_history.append(train_recon_averager(None))
        ...
        val_l1_averager = make_averager()
        val_mse_averager = make_averager()
        val_psnr_averager = make_averager()
        val_ssim_averager = make_averager()
        ...
        with torch.no_grad():
            ...
            for image_batch, _ in val_bar:
                ...
                metrics = compute_image_metrics(x_recon.cpu(), image_batch.cpu())
                val_l1_averager(metrics['l1'])
                val_mse_averager(metrics['mse'])
                val_psnr_averager(metrics['psnr'])
                val_ssim_averager(metrics['ssim'])

                refresh_bar(
                    val_bar, f"validation batch [loss: {val_loss_averager(None):.3f} | recon loss: {val_recon_averager(None):.3f}]")
        ...
        with torch.no_grad():
            ...
            wandb.log({
                ...
                # Other metrics
                "val/l1": val_l1_averager(None),
                "val/mse": val_mse_averager(None),
                "val/psnr": val_psnr_averager(None),
                "val/ssim": val_ssim_averager(None),
        ...
        # Save checkpoint periodically even if early stopping is triggered
...
def fit_vqvae(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for image_batch, _ in batch_bar:
            ...
            train_loss_averager(loss)
            train_recon_averager(recon_loss)
            train_vq_averager(vq_loss)
            refresh_bar(
                batch_bar, f"train batch [loss: {train_loss_averager(None):.3f} | recon loss: {train_recon_averager(None):.3f} | vq: {train_vq_averager(None):.3f}]")

        train_loss_avg_history.append(train_loss_averager(None))
        train_recon_avg_history.append(train_recon_averager(None))
        train_vq_averager = train_vq_averager(None)
        ...
        print(
            f"Epoch {epoch}: "
            f"Train Loss={train_loss_avg_history[-1]:.4f} (recon loss={train_recon_avg_history[-1]:.4f}, vq={train_vq_averager:.4f})\n"
        ...
        with torch.no_grad():
            ...
            wandb.log({
                ...
                "train/loss": train_loss_avg_history[-1],
                "train/recon_loss": train_recon_avg_history[-1],
                "train/vq_loss": train_vq_averager,
                "val/loss": val_loss_avg_history[-1],
                "val/recon_loss": val_recon_avg_history[-1],
...
def step_pixelcnn(model, loss_fn, xb, device, opt=None):
    codes_batch = xb.to(device) # (B, H, W) codes indices
    logits = model(codes_batch) # (B, K, H, W)
    loss = loss_fn(logits, codes_batch)
    if opt is not None:
        opt.zero_grad()
        loss.backward()
        opt.step()
...
def fit_pixelcnn(
    ...
    fixed_batch = None  # To store a fixed batch for visualization
    model = model.to(device)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for codes_batch in batch_bar:
            loss, logits = step_pixelcnn(
                model, F.cross_entropy, codes_batch, device, optimizer
            )
        ...
        with torch.no_grad():
            ...
            for codes_batch in val_bar:
                loss, logits = step_pixelcnn(
                    model, F.cross_entropy, codes_batch, device, opt=None
                )
                val_loss_averager(loss)
