"""
Interpolate between latent codes of two images and compare reconstruction quality (CelebA).
...
from vq_vae_geodesic.config import samples_dir
...
from vq_vae_geodesic.evaluation.utils import (
    load_model_vae_celeba,
    load_model_vqvae_celeba,
    load_codebook_celeba,
    lookup_codewords
)

IMG_SIZE = 32  # FIXED at 32
...
def run_interpolation():
    ...
    vae = load_model_vae_celeba(config.arch_params, device)
    codebook_chunks = load_codebook_celeba(device)
    vqvae = load_model_vqvae_celeba(config.arch_params, config.vqvae_params, device)
    print("Models loaded\n")
    
    # Get data (NOT shuffled for reproducible selection)
    print("Loading data...")
    ...
    for i in range(n_steps):
        ax = axes[0, i]
        # For CelebA RGB, need to permute (C, H, W) → (H, W, C) for display
        if i == 0:
            img_display = img1[0].cpu().permute(1, 2, 0).numpy()
            ...
            img_display = img2[0].cpu().permute(1, 2, 0).numpy()
            ax.imshow(img_display)
            ax.set_title('Image 2', fontsize=9)
        else:
            # Empty middle cells
            ax.imshow(np.ones((IMG_SIZE, IMG_SIZE, 3)) * 0.5)
    ...
    plt.suptitle('Latent Code Interpolation (CelebA)', fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    plot_path = save_dir / "interpolation_data_celeba.png"
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"Saved: {plot_path}")
    
    # Save data
    data_path = save_dir / "interpolation_data_celeba.pt"
    torch.save({
        'img1': img1.cpu(),
        'img2': img2.cpu(),
