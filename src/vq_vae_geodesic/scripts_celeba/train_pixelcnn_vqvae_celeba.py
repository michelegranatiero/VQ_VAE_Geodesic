...
"""Train PixelCNN autoregressive prior on VQ-VAE quantized CelebA codes."""
def launch_train_pixelcnn_vqvae(resume=False):
    ...
    checkpoint_path_vqvae = checkpoint_dir('celeba') / "vqvae_celeba_best.pt"
    checkpoint = torch.load(checkpoint_path_vqvae, map_location=device)
    ...
    vqvae.eval()
    print("Loaded VQ-VAE model for CelebA.")

    # Use pre-extracted VQ-VAE assigned-like codes
    codes_dir = latents_dir('celeba')
    assigned_codes_path = codes_dir / 'vqvae_assigned_codes_celeba.pt'
    if not assigned_codes_path.exists():
        raise FileNotFoundError(f"Expected {assigned_codes_path} (run extract_vqvae_codes_celeba first)")
    ...
    train_loss_history = []
    val_loss_history = []
    checkpoint_path = checkpoint_dir('celeba') / "pixelcnn_vqvae_celeba.pt"
    ...
    print(f"\nFinal model saved in {checkpoint_dir('celeba')}")
