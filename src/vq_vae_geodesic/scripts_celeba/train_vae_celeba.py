...
from vq_vae_geodesic.training.losses import vae_loss_mse
...
def launch_train(resume=False, img_size=32):
    ...
    # Build model
    model = build_vae_from_config(config.arch_params, dataset="celeba")
    ...
    loss_fn = partial(vae_loss_mse, beta=config.training_params.variational_beta)
    ...
    val_loss_history = []
    val_recon_history = []
    checkpoint_path = checkpoint_dir('celeba') / "vae_celeba.pt"
    ...
    print("uv run -m vq_vae_geodesic.scripts_celeba.extract_celeba_latents")

if __name__ == "__main__":
    launch_train(resume=RESUME, img_size=IMG_SIZE)
