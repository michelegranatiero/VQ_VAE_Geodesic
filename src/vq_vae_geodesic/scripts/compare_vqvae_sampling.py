"""
Compare VQ-VAE sampling methods: PixelCNN vs Random
"""
import torch
...
def compare_vqvae_sampling_methods(n_samples=16, temperature=1.0):
    ...
    # VQ-VAE uses 7x7 spatial latent grid
    vqvae_grid_shape = (7, 7)
    ...
    # Save visualization
    plot_path = save_dir / "vqvae_pixelcnn_vs_random.png"
    plot_comparison_2rows(
        ...
        save_path=plot_path,
        title="VQ-VAE: End-to-End Learning with PixelCNN vs Random Sampling",
        n_show=16
    )
    
    # Save data as torch
    data_path = save_dir / "vqvae_pixelcnn_vs_random.pt"
    torch.save({
