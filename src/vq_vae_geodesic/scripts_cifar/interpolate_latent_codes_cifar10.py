"""
Interpolate between latent codes of two images and compare reconstruction quality (CIFAR-10).
...
def run_interpolation():
    ...
    print("Loading data...")
    train_loader, _, _ = get_cifar_loaders(batch_size=64, shuffle_train_set=False)
    ...
    for i in range(n_steps):
        ax = axes[0, i]
        # For CIFAR-10, need to permute (C, H, W) → (H, W, C) for display
        if i == 0:
    ...
    plt.suptitle('Latent Code Interpolation (CIFAR-10)', fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    plot_path = save_dir / "interpolation_data_cifar10.png"
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"Saved: {plot_path}")
    
    # Save data
    data_path = save_dir / "interpolation_data_cifar10.pt"
    torch.save({
        'img1': img1.cpu(),
        'img2': img2.cpu(),
