...
def launch_quantization():
    ...
    # -------------- LATENTS LOADING --------------
    train_latents_path = latents_dir('cifar10') / "train_latents_cifar10.pt"
    ...
    # Save codebook
    save_path = latents_dir('cifar10') / "chunk_codebook_cifar10.pt"
    quantizer.save(save_path)
    print(f"\nCodebook saved to {save_path}")

    # Save all codes together for convenience
    all_codes_path = latents_dir('cifar10') / "assigned_codes_cifar10.pt"
    torch.save({
    ...
    print(f"Codes saved to {latents_dir('cifar10')}")
