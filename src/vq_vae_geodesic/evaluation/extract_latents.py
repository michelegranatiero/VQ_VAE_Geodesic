...
def extract_and_save_latents(model, dataloader, device, save_dir, prefix):
    ...
    the resulting mu and logvar tensors as a compressed .npz file.
    ...
    Returns:
        out_path: Path to the saved .npz file
    """
    model.eval()
    mus = []
    logvars = []
    ...
    # Save to disk
    out_path = save_dir / f"{prefix}_latents.npz"
    np.savez_compressed(out_path, mu=mus, logvar=logvars) # Keys are 'mu' and 'logvar'
